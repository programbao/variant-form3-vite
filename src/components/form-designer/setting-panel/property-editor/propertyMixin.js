export default {
  methods: {
    hasConfig(e) {
        return !this.designer || !this.designer.selectedWidget ? !1 : this.designer.hasConfig(this.selectedWidget, e)
    },
    emitDefaultValueChange() {
        if (this.designer && this.designer.formWidget) {
            let e = this.designer.formWidget.getWidgetRef(this.designer.selectedWidget.options.name);
            !!e && !!e.refreshDefaultValue && e.refreshDefaultValue()
        }
    },
    ...
    onRemoteChange(e) {
        e && (this.optionModel.filterable = !0)
    },
    onAllowCreateChange(e) {
        e && (this.optionModel.filterable = !0)
    },
    onMultipleSelected(e) {
        if (e) {
            let t = this.designer.formWidget.getWidgetRef(this.optionModel.name);
            !!t && !!t.clearSelectedOptions && t.clearSelectedOptions(),
            this.optionModel.defaultValue = []
        } else
            !!this.optionModel.defaultValue && this.optionModel.defaultValue.length > 0 ? this.optionModel.defaultValue = this.optionModel.defaultValue[0] : this.optionModel.defaultValue = "";
        const o = this.designer.formWidget.getSelectedWidgetRef();
        o && o.refreshWidgetKey && o.refreshWidgetKey()
    }
}
  // methods: {
