        ...
        <el-scrollbar class="setting-scrollbar">
          ...
          >
            ...
            >
              ...
              >
                <template
                  v-for="(prop, key) in commonProps"
                  :key="key"
                >
                  <component
                    v-if="hasPropEditor(key, prop)"
                    :is="getPropEditor(key, prop)"
                    :designer="designer"
...
export default {
  ...
  methods: {
    ...
    showEventCollapse() {
      return this.designerConfig.eventCollapse === void 0
        ? !0
        : !!this.designerConfig.eventCollapse
    },
    hasPropEditor(e, o) {
      //console.log(e, o, '820348290384')
      if (!o) return !1
      if (e.indexOf('-') <= -1) {
        let r = this.selectedWidget.type + '-' + e
        if (propertyRegistered(r)) return !1
      }
      let t = e.replace(this.selectedWidget.type + '-', '')
      return this.designer.hasConfig(this.selectedWidget, t)
    },
    getPropEditor(e, o) {
      let t = e.replace(this.selectedWidget.type + '-', ''),
    ...
    showCollapse(e) {
      let o = !1
      for (let t in e)
        if (!!e.hasOwnProperty(t) && this.hasPropEditor(t, e[t])) {
          o = !0
          break
        }
    ...
    editEventHandler(e, o) {
      ;(this.curEventName = e),
        (this.eventHeader = `${this.optionModel.name}.${e}(${o.join(', ')}) {`),
        (this.eventHandlerCode = this.selectedWidget.options[e] || ''),
        e === 'onValidate' &&
  ...
  */`),
    ...
    saveEventHandler() {
      const e = this.$refs.ecEditor.getEditorAnnotations()
      let o = !1
      if (
        !!e &&
        e.length > 0 &&
...
:deep(.setting-scrollbar) .el-scrollbar__wrap,:deep(.ds-setting-scrollbar) .el-scrollbar__wrap {
    overflow-x: hidden
}

:deep(.setting-collapse) .el-collapse-item__content {
    padding-bottom: 6px
}

:deep(.setting-collapse) .el-collapse-item__header {
    font-style: italic;
    font-weight: 700
}
...
.setting-form .el-form-item--small.el-form-item {
    margin-bottom: 10px
}

 .hide-spin-button input::-webkit-outer-spin-button, .hide-spin-button input::-webkit-inner-spin-button {
...
:deep(.small-padding-dialog) .el-dialog__body {
